...
# ===== Cấu hình =====
input_dir = './Malicious-PowerShell-Dataset/hybrid_analysis_out'  # Thư mục chứa các file
output_csv = './dataset/Malicious-PowerShell-Dataset.csv'  # File CSV để lưu kết quả
unknown_csv = './checkpoint/Malicious-PowerShell-Dataset/Malicious-PowerShell-Dataset_label_unknown.csv'  # File CSV cho các mẫu unknown
checkpoint_file = './checkpoint/Malicious-PowerShell-Dataset/Malicious-PowerShell-Dataset_checkpoint.txt'
...
# ===== Khởi tạo =====
ps1_files = [f for f in os.listdir(input_dir) if f.endswith('.ps1') or f.endswith('.bin.sample')]  # Xử lý cả .ps1 và .bin.sample
results = []
unknowns = []
api_index = 0
...
for fname in ps1_files:
    ...
    if label == 'Unknown':
        unknowns.append(row_df)  # Ghi vào danh sách unknowns
    else:
        results.append(row_df)  # Ghi vào danh sách results
...
# ===== Lưu kết quả =====
if results:
    df = pd.concat(results, ignore_index=True)  # Kết hợp tất cả các kết quả
    df.to_csv(output_csv, index=False, escapechar='\\')
    print(f"✅ Đã lưu {len(results)} nhãn hợp lệ → {output_csv}")

if unknowns:
    df_unknown = pd.concat(unknowns, ignore_index=True)  # Kết hợp tất cả các unknowns
    df_unknown.to_csv(unknown_csv, index=False, escapechar='\\')
